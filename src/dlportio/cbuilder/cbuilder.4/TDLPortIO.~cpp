...
void __fastcall TDLPrinterPortIO::DetectPorts()
{
   ...
   if (RunningWinNT)
      if (os.dwMajorVersion >= 5)
         DetectPorts2000();  // Win2000 version
      else
...
// DetectPorts2000()
//---------------------------------------------------------------------------
void __fastcall TDLPrinterPortIO::DetectPorts2000()
{
   const char *BASE_KEY = "HARDWARE\\DESCRIPTION\\System\\MultifunctionAdapter";
   const char *PARALLELCONTROLLER = "ParallelController";
   const char *CONFIGURATIONDATA = "Configuration Data";
   ...
   // Grab all the key names under HKEY_LOCAL_MACHINE
   //
   // Do this by first counting the number of keys,
   // then creating an array big enough to hold them
   // using the KeyList pointer.
   ...
   // Nos recorremos todas las entradas buscando aquellas que contengan
   // la descripción de un controlador de puerto paralelo
   for (DWORD KeyIndex=0; KeyIndex<KeyCount; KeyIndex++)
   {
      // Open the key
      strcpy(KeyName, BASE_KEY);
      ...
      strcat(KeyName, KeyList[KeyIndex]);
      strcat(KeyName, "\\");
      strcat(KeyName, PARALLELCONTROLLER);
      if (RegOpenKeyEx(
            HKEY_LOCAL_MACHINE, KeyName, 0, KEY_PERMISSIONS, &CurKey
                        ) != ERROR_SUCCESS)
         continue;

      char PartialKeyName[MAX_PATH];    // A key name when using the registry
      char CompleteKeyName[MAX_PATH];    // A key name when using the registry
      DWORD KeyCount2;            // Count of the number of keys in KeyList2

      // Obtenemos la lista de puertos de cada controlador...
      DummyLength = MAX_PATH;
      KeyCount2 = 0;
      while (RegEnumKeyEx(
                CurKey, KeyCount2, PartialKeyName, &DummyLength,
                NULL, NULL, NULL, &DummyFileTime
                            ) != ERROR_NO_MORE_ITEMS)
      {
         strcpy(CompleteKeyName, KeyName);
         strcat(CompleteKeyName, "\\");
         strcat(CompleteKeyName, PartialKeyName);

         HKEY CurKey2;               // Current key when using the registry

         // ...y obtenemos información de cada uno de ellos
         if (RegOpenKeyEx(
            HKEY_LOCAL_MACHINE, CompleteKeyName, 0, KEY_PERMISSIONS, &CurKey2
                        ) != ERROR_SUCCESS)
         continue;

         DWORD DataType, DataSize;

         RegQueryValueEx(CurKey2, CONFIGURATIONDATA, NULL, &DataType, NULL, &DataSize);
         if (DataType == REG_FULL_RESOURCE_DESCRIPTOR)
         {
            BYTE *Data = new BYTE[DataSize];

            if (RegQueryValueEx(
                  CurKey2, CONFIGURATIONDATA, NULL,
                  NULL, Data, &DataSize
                             ) == ERROR_SUCCESS)
            {
                // Guardamos la dirección base del puerto
                // Numeramos los puertos en el orden en que los hemos encontrado
                // (puede no coincidir con los nombres asignados por windows)
                FLPTCount++;
                FLPTAddress[FLPTCount] = Data[0x14]+(Data[0x15]<<8);
            }
            delete[] Data;
         }
         RegCloseKey(CurKey2);
